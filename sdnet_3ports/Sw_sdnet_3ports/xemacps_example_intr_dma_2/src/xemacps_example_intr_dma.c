...
// Ethernet Subsystem Addr

#define  ADDR_GT_RESET_REG                        0x00000000
#define  ADDR_MODE_REG                            0x00000008
#define  ADDR_CONFIG_TX_REG1                      0x0000000C
#define  ADDR_CONFIG_RX_REG1                      0x00000014
#define  ADDR_TICK_REG                            0x00000020
...
void configEthSub(void){
	Xil_Out32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_MODE_REG, 0x40000000);
	Xil_Out32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_CONFIG_RX_REG1, 0x00000033);
	Xil_Out32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_CONFIG_TX_REG1, 0x00003003);
	Xil_Out32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_GT_RESET_REG, 0x00000000);
	Xil_Out32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_TICK_REG, 0x00000001);


	Xil_Out32(0x80000000 + ADDR_MODE_REG, 0x40000000);
	Xil_Out32(0x80000000 + ADDR_CONFIG_RX_REG1, 0x00000033);
	Xil_Out32(0x80000000 + ADDR_CONFIG_TX_REG1, 0x00003003);
	Xil_Out32(0x80000000 + ADDR_GT_RESET_REG, 0x00000000);
	Xil_Out32(0x80000000 + ADDR_TICK_REG, 0x00000001);

	unsigned int temp0 = Xil_In32(XPAR_XXV_ETHERNET_0_BASEADDR + ADDR_CONFIG_TX_REG1);

	xil_printf("ADDR_CONFIG_TX_REG1 - 0 : %x\n\r", temp0);

	unsigned int temp1 = Xil_In32(0x80000000 + ADDR_CONFIG_TX_REG1);

	xil_printf("ADDR_CONFIG_TX_REG1 - 1  : %x\n\r", temp1);
			  ...
			  u16 EmacPsIntrId)
{
	...
	//Enable full duplex. Write a 1 to the gem.network_config[full_duplex] bit.

//	XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
//						0x00000004,
//						XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000004) | 0x2);


	XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
						0x00000004,
						XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000004) & ~0x600000);
...
//	}
	...
	//write 1 in GEM3_FIFO_CLK_SEL

	XEmacPs_WriteReg(0xFF180308,
			0x00000000,
			XEmacPs_ReadReg(0xFF180308,0x00000000) | 0x40000);
	...
	XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
					0x00000000,
					XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000000) | 0xC);

	//enable copy all frames, try to prevent address problem

	XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
							0x00000004,
							XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000004) | 0x10);


	unsigned int gem_clk_ctrl = XEmacPs_ReadReg(0xFF180308, 0x00000000);

	//xil_printf("gem clk crl : %x\n\r", gem_clk_ctrl);

	unsigned int net_ctrl = XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR, 0x00000000);

	//xil_printf("net_ctrl : %x\n\r", net_ctrl);

	unsigned int net_config = XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR, 0x00000004);

	//xil_printf("net_config : %x\n\r", net_config);

	unsigned int gem_status = XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR, 0x00000008);

	//xil_printf("gem_status : %x\n\r", gem_status);

	//xil_printf("GEM VERSION : %d\n\r", GemVersion);
	...
	XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
						0x00000000,
						XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000000) | 0x200);

	unsigned int frames_tx;

	while(1){
//		if ( (XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000000) & 0x200) == 0 ){
//			XEmacPs_WriteReg(XPAR_XEMACPS_0_BASEADDR,
//				0x00000000,
//				XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR,0x00000000) | 0x200);
//		}
		//frames_tx = XEmacPs_ReadReg(XPAR_XEMACPS_0_BASEADDR, 0x00000108);
		//xil_printf("frames_tx : %x\n\r", frames_tx);
